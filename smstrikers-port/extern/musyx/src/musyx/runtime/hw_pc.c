...
void hwInitIrq() {
  ...
  /* PORT/Vita: public MusyX APIs use hwDisableIrq()/hwEnableIrq() as their
     interrupt exclusion primitive. Once the software DSP runs on another
     thread that primitive must become a real recursive lock. hwInit() starts
     with interrupts disabled, so take the initial level here; its existing
     hwEnableIrq() releases it immediately before salStartAi(). */
