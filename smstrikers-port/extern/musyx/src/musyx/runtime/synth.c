    ...
    if (
      ...
      if ((sv->cFlags & 0x10) != 0 && (sv->cFlags & 0x10000000008) != 0x8 && hwIsActive(i)) {
        ...
        if (vid == SND_ID_ERROR) {
          ...
          /* PORT: the vid pool is finite and the references have already been stripped. */
          if (vid == SND_ID_ERROR) {
            if (hwIsActive(i)) {
              hwBreak(i);
            }
            voiceFree(&synthVoice[i]);
            continue;
          }
          id = sv->id;
        } else {
          ...
          sv->parent = id;
          /* PORT: same failure, same answer, and `id` is still the voice whose child was just set. */
          if (vidMakeNew(&synthVoice[i], FALSE) == SND_ID_ERROR) {
            synthVoice[id & 0xff].child = SND_ID_ERROR;
            sv->parent = SND_ID_ERROR;
            if (hwIsActive(i)) {
              hwBreak(i);
            }
